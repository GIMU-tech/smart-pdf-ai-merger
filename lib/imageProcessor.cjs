...
function normalizeSplitOptions(options = {}) {
  ...
  const strategy = ['fixed', 'manual', 'ai-flow'].includes(options.strategy) ? options.strategy : 'flow';
  ...
  if (options.manualCutsByFile && typeof options.manualCutsByFile === 'object') {
    for (const [fileName, cuts] of Object.entries(options.manualCutsByFile)) {
      if (!Array.isArray(cuts)) continue;
      manualCutsByFile[fileName] = Array.from(new Set(
        cuts
          .map(value => positiveInteger(value))
          .filter(value => value && value > 0)
      )).sort((a, b) => a - b);
...
function findFlowCutNear(analysis, desiredCut, totalPixels, options, previousCut, nextCut) {
  const minGap = Math.max(1, options.minLastChunkPixels);
  const requestedWindow = Math.max(80, options.searchWindow);
  const window = Math.max(requestedWindow, Math.min(900, Math.round(options.maxPixels * 0.3)));
  const minCut = Math.max(1, previousCut + minGap, desiredCut - window);
  const maxCut = Math.min(totalPixels - 1, nextCut - minGap, desiredCut + window);

  if (maxCut <= minCut || analysis.axisScale <= 0) {
    return clamp(desiredCut, minCut, Math.max(minCut, maxCut));
  }

  const sampleMin = Math.max(3, Math.floor(minCut * analysis.axisScale));
  const sampleMax = Math.min(
    options.axis === 'vertical' ? analysis.height - 4 : analysis.width - 4,
    Math.ceil(maxCut * analysis.axisScale)
  );

  if (sampleMax <= sampleMin) {
    return clamp(desiredCut, minCut, maxCut);
  }

  const step = Math.max(1, Math.floor((sampleMax - sampleMin) / 180));
  let bestSample = Math.round(clamp(desiredCut, minCut, maxCut) * analysis.axisScale);
  let bestScore = Number.POSITIVE_INFINITY;

  for (let sample = sampleMin; sample <= sampleMax; sample += step) {
    const originalCut = clamp(Math.round(sample / analysis.axisScale), minCut, maxCut);
    const distancePenalty = (Math.abs(originalCut - desiredCut) / Math.max(1, window)) * 5;
    const score = bandScore(analysis, options.axis, sample) + distancePenalty;
  ...
  return clamp(Math.round(bestSample / analysis.axisScale), minCut, maxCut);
}

function enforceMaxRangeLengthWithFlow(analysis, cuts, totalPixels, options) {
  const hardCuts = [];
  const boundaries = [...cuts, totalPixels];
  let start = 0;

  for (const boundary of boundaries) {
    let segmentEnd = boundary;

    while (segmentEnd - start > options.maxPixels) {
      const segmentLength = segmentEnd - start;
      const desiredCut =
        segmentLength <= options.maxPixels * 2
          ? start + Math.round(segmentLength / 2)
          : start + options.maxPixels;
      const remainingAfterMaxCut = segmentEnd - desiredCut;
      let nextCut;

      if (segmentLength <= options.maxPixels * 2) {
        nextCut = findFlowCutNear(analysis, desiredCut, segmentEnd, options, start, segmentEnd);
      } else if (remainingAfterMaxCut > 0 && remainingAfterMaxCut < options.minLastChunkPixels) {
        nextCut = findFlowCutNear(analysis, start + Math.round(segmentLength / 2), segmentEnd, options, start, segmentEnd);
      } else {
        nextCut = findFlowCutEnd(analysis, start, desiredCut, segmentEnd, options);
      }

      nextCut = clamp(nextCut, start + options.minLastChunkPixels, Math.min(start + options.maxPixels, segmentEnd - options.minLastChunkPixels));
      if (nextCut <= start || nextCut >= segmentEnd) break;

      hardCuts.push(nextCut);
      start = nextCut;
    }

    if (boundary < totalPixels) {
      if (boundary - start >= options.minLastChunkPixels && boundary - start <= options.maxPixels) {
        hardCuts.push(boundary);
        start = boundary;
      }
    }
  }

  return Array.from(new Set(hardCuts)).sort((a, b) => a - b);
}

function rangesToCuts(ranges) {
  const cuts = [];
  let cursor = 0;
  for (let index = 0; index < ranges.length - 1; index += 1) {
    cursor += ranges[index].length;
    cuts.push(cursor);
  }
  return cuts;
}

function mergeSmallRanges(cuts, totalPixels, options) {
  const minMeaningfulLength = Math.max(options.minLastChunkPixels * 2, 700);
  const ranges = splitRangesByManual(totalPixels, cuts);
  let changed = true;

  while (changed) {
    changed = false;
    for (let index = 0; index < ranges.length; index += 1) {
      if (ranges[index].length >= minMeaningfulLength) continue;

      const prev = ranges[index - 1];
      const next = ranges[index + 1];
      const canMergePrev = prev && prev.length + ranges[index].length <= options.maxPixels;
      const canMergeNext = next && next.length + ranges[index].length <= options.maxPixels;

      if (!canMergePrev && !canMergeNext) continue;

      if (canMergePrev && (!canMergeNext || prev.length <= next.length)) {
        prev.length += ranges[index].length;
        ranges.splice(index, 1);
      } else {
        ranges[index].length += next.length;
  ...
  return rangesToCuts(ranges);
}

async function splitRangesByAiCuts(inputPath, meta, totalPixels, manualCuts, options) {
  if (!manualCuts.length) {
    return splitRangesByFlow(inputPath, meta, totalPixels, options);
  }

  const analysis = await buildSplitAnalysis(inputPath, meta, options);
  const snappedCuts = [];

  for (let index = 0; index < manualCuts.length; index += 1) {
    const previousCut = snappedCuts[snappedCuts.length - 1] || 0;
    const nextCut = manualCuts[index + 1] || totalPixels;
    const desiredCut = manualCuts[index];
    const snappedCut = findFlowCutNear(analysis, desiredCut, totalPixels, options, previousCut, nextCut);

    if (snappedCut > previousCut && totalPixels - snappedCut >= options.minLastChunkPixels) {
      snappedCuts.push(snappedCut);
    }
  }

  const hardCuts = mergeSmallRanges(
    enforceMaxRangeLengthWithFlow(analysis, snappedCuts, totalPixels, options),
    totalPixels,
    options
  );
  return splitRangesByManual(totalPixels, hardCuts);
}
...
async function splitImages({ inputPaths = [], outputDir, options = {} }) {
  ...
  for (const inputPath of inputPaths) {
    ...
    const baseName = sanitizeBaseName(path.basename(inputPath));
    const aiManualCuts = normalizedOptions.manualCutsByFile[path.basename(inputPath)] || normalizedOptions.manualCutsByFile[baseName] || [];
    const ranges =
      normalizedOptions.strategy === 'manual'
        ? splitRangesByManual(totalPixels, normalizedOptions.manualCuts)
        : normalizedOptions.strategy === 'ai-flow'
        ? (aiManualCuts.length > 0
            ? splitRangesByManual(totalPixels, aiManualCuts)
